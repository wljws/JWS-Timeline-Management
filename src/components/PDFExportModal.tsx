...
export const PDFExportModal: React.FC<PDFExportModalProps> = ({ projects, phaseColors, onClose }) => {
  ...
  const [zoomLevel, setZoomLevel] = useState<number>(20); // Default adjusted to be less stretched
  ...
  const months = useMemo(() => {
    const list = [];
    if (isNaN(timelineStart.getTime()) || isNaN(timelineEnd.getTime())) return [];
  ...
  const renderTimeline = (isForExport: boolean = false) => (
    <div style={{ width: `${gridWidth + 300}px`, backgroundColor: 'white', padding: isForExport ? '40px' : '20px', fontFamily: '"Inter", sans-serif' }}>
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: isForExport ? '24px' : '18px', fontWeight: 'bold', color: '#1e293b' }}>Project Timeline Report</h1>
        <div style={{ fontSize: isForExport ? '12px' : '10px', color: '#64748b' }}>Generated on {new Date().toLocaleDateString()}</div>
      </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Controls Panel */}
          <div className="w-80 border-right border-slate-100 p-6 space-y-6 overflow-y-auto bg-slate-50/50">
            ...
            <div className="space-y-2">
               <div className="flex justify-between">
                 <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Column Width (Density)</label>
                 <span className="text-xs font-mono text-slate-400">{zoomLevel}px</span>
               </div>
               <input 
                 type="range"
                 min="5"
               ...
               />
               <p className="text-[10px] text-slate-400">Decrease width to fit more timeline on one page.</p>
            </div>
            ...
            <div className="pt-6">
              ...
              >
                {isExporting ? <><Icons.Spinner className="animate-spin" /> Generating...</> : <><Icons.Download /> Export PDF</>}
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 bg-slate-200 p-8 overflow-auto flex justify-center items-start">
            <div className="shadow-2xl bg-white min-w-max">
