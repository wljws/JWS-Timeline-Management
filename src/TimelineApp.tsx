...
export const TimelineApp: React.FC<TimelineAppProps> = ({ onLogout, userRole }) => {
  ...
  // Vercel KV Config
  const [appId] = useState('timeline-app-data');
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.history && Array.isArray(data.history) && data.history.length > 0) {
             applyLoadedData({ collections: data.history });
  ...
  // Sync Logics
  useEffect(() => {
    const VERCEL_KV_URL = import.meta.env.VITE_VERCEL_KV_REST_API_URL;
    const VERCEL_KV_TOKEN = import.meta.env.VITE_VERCEL_KV_REST_API_TOKEN;
    ...
    const fetchKV = async (isInit = false) => {
      try {
        const res = await fetch(VERCEL_KV_URL, {
          method: 'POST',
          headers: { Authorization: `Bearer ${VERCEL_KV_TOKEN}`, 'Content-Type': 'application/json' },
          body: JSON.stringify(["GET", appId])
        });
        const json = await res.json();
        if (json.result) {
          const data = typeof json.result === 'string' ? JSON.parse(json.result) : json.result;
          if (isInit || (data.timestamp && data.timestamp > lastSavedTimestampRef.current)) {
            if (!viewingSnapshot) {
      ...
      } catch (e) { console.error(e); } finally { if (isInit) setIsDataLoaded(true); }
    };
    fetchKV(true);
    const poll = setInterval(() => fetchKV(false), 3000);
    return () => clearInterval(poll);
  }, [appId, viewingSnapshot]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const VERCEL_KV_URL = import.meta.env.VITE_VERCEL_KV_REST_API_URL;
      const VERCEL_KV_TOKEN = import.meta.env.VITE_VERCEL_KV_REST_API_TOKEN;
      if (!VERCEL_KV_URL || !VERCEL_KV_TOKEN) return;
      ...
      try {
        await fetch(VERCEL_KV_URL, {
          method: 'POST',
          headers: { Authorization: `Bearer ${VERCEL_KV_TOKEN}`, 'Content-Type': 'application/json' },
          body: JSON.stringify(["SET", appId, JSON.stringify({ collections: JSON.stringify(collections), timestamp: ts })]),
        });
        setSyncStatus('Synced');
      } catch (e) { setSyncStatus('Failed'); }
    }, 1500);
    return () => clearTimeout(timer);
  }, [collections, isDataLoaded, appId, actualIsReadOnly, viewingSnapshot]);
